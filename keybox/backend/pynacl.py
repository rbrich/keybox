from .common import BaseScryptParams, BaseArgon2Params
...
class ScryptParams(BaseScryptParams):

    def __init__(self,
                 mem_cost: int = BaseScryptParams.DEFAULT_MEM_COST,
                 time_cost: int = BaseScryptParams.DEFAULT_TIME_COST):
        BaseScryptParams.__init__(self, mem_cost, time_cost)
...
def scrypt(secret: bytes, salt: bytes, hash_len: int, params: ScryptParams()):
    return nacl.pwhash.scrypt.kdf(hash_len, secret, salt,
                                  2 ** params.time_cost, 2 ** (10 + params.mem_cost))
